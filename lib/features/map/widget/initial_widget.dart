...
import 'package:ride_sharing_user_app/features/ride/widgets/seat_selection_widget.dart';
    ...
    return GetBuilder<RideController>(builder: (rideController){
      return GetBuilder<LocationController>(builder: (locationController){
        return Column(mainAxisSize: MainAxisSize.min, children:  [
          ...
          // Seat Selection Widget - Only for Car and Car Mini
          if(rideController.categoryName.isNotEmpty && rideController.categoryName.toLowerCase().contains('car') && !rideController.categoryName.toLowerCase().contains('parcel'))
            ...[              
              SeatSelectionWidget(
                seatShareModel: SeatShareModel(
                  id: rideController.categoryName,
                  vehicleType: rideController.categoryName.toLowerCase().contains('mini') ? 'car_mini' : 'car',
                  totalSeats: 4,
                  seats: [],
                  pricePerSeat: rideController.seatSharePrice,
                ),
                onSeatSelected: (seatIndex) {
                  if(rideController.fareList.isNotEmpty && rideController.rideCategoryIndex < rideController.fareList.length) {
                    double seatPrice = (rideController.fareList[rideController.rideCategoryIndex].estimatedFare ?? 0.0) * 0.5;
                    rideController.setSelectedSeat(seatIndex, seatPrice);
                  }
                },
                selectedSeat: rideController.selectedSeat,
              ),
              const SizedBox(height: Dimensions.paddingSizeDefault),
            ],
