...
class MessageController extends GetxController implements GetxService{
  ...
  Timer? _typingTimer;
  static const Duration _typingIndicatorDuration = Duration(seconds: 3);
  ...
  void sendTypingIndicator(String channelId) {
    ...
    try {
      // Check if channel is subscribed (using the actual channel object)
      if (channel.currentStatus == null) {
        return;
      }

      // Note: The dart_pusher_channels package may have different API
      // Let's check the actual API by looking at your existing code pattern
      // In your subscribeMessageChannel, you use channel.bind() which returns a Stream
      // Let's use a safer approach

      // First, let's check how your PusherHelper triggers events
      // Looking at pusher_helper.dart, you don't use channel.trigger() there
      // So let's simplify and just set a local typing indicator for now
      // The actual Pusher implementation might need to be different

      // For now, let's just update the UI locally
      // We'll come back to Pusher implementation once we confirm the API
    ...
    // Reset typing indicator after duration
    _typingTimer = Timer(_typingIndicatorDuration, () {
      sendStoppedTypingIndicator(channelId);
    });
  }

  void sendStoppedTypingIndicator(String channelId) {
    _typingTimer?.cancel();
    try {
      print('User stopped typing...');
    } catch (e) {
      print('Error sending stopped typing indicator: $e');
    }
  }
  ...
  void disposeTypingIndicator() {
    _typingTimer?.cancel();
  }

  @override
  void onClose() {
    disposeTypingIndicator();
    super.onClose();
  }
