...
// Persistence keys
const String ACTIVE_RIDE_KEY = 'active_ride_data';
const String ACTIVE_RIDE_STATE_KEY = 'active_ride_state';
const String ACTIVE_RIDE_ID_KEY = 'active_ride_id';
...
class RideController extends GetxController implements GetxService {
  ...
  void setSelectedSeat(int seatIndex, double price) {
    selectedSeat = seatIndex;
    seatSharePrice = price;
    isSharedRide = seatIndex >= 0;
    if (isSharedRide) {
      actualFare = estimatedFare + seatSharePrice;
      currentFarePrice = actualFare;
    } else {
  ...
  void setRideCategoryIndex(int newIndex){
    ...
    // Reset seat selection when category changes
    resetSeatSelection();
  ...
  Future<Response> submitRideRequest(String note, bool parcel, {String categoryId = ''}) async {
    ...
    Response response = await rideServiceInterface.submitRideRequest(
      ...
      customerCurrentLng: locController.initialPosition.longitude.toString(), type: parcel ? 'parcel' : 'ride_request',
        rideRequestType: parcel ? null : _rideType == RideType.regularRide ? 'regular' : 'scheduled',
      selectedSeat: parcel ? null : selectedSeat >= 0 ? selectedSeat : null,
      ...
      estimatedFare: parcel ? parcelFare : (isSharedRide ? actualFare : estimatedFare).toString(),
      actualFare: parcel ? parcelFare : (isSharedRide ? actualFare : estimatedFare).toString(),
  ...
  Future<Response> getFinalFare(String tripId) async {
    ...
    if (response.statusCode == 200 ) {
      if(response.body['data'] != null){
        ...
        // Debit wallet after fare is finalized
        if((finalFare?.paidFare ?? 0.0) > 0){
          _debitWalletForRide(tripId, finalFare?.paidFare ?? 0.0);
        }
      }
    }else{
      ApiChecker.checkApi(response);
  ...
  Future<void> _debitWalletForRide(String tripId, double amount) async {
    try {
      final walletController = Get.find<WalletController>();
      Response response = await walletController.debitWallet(tripId, amount);
      
      if (response.statusCode == 200) {
        //displaySuccessSnackbar('ride_payment_successful'.tr);
        showCustomSnackBar('ride_payment_successful'.tr, isError: false);
        // Clear persisted ride state after successful payment
        await clearPersistedRideState();
      } else if (response.statusCode == 402) {
        // Insufficient balance
        //displayErrorSnackbar('insufficient_wallet_balance'.tr);
        showCustomSnackBar('insufficient_wallet_balance'.tr, isError: true);
      }
    } catch (e) {
      //displayErrorSnackbar('wallet_debit_error'.tr);
      showCustomSnackBar('wallet_debit_error'.tr, isError: true);
    }
  }
  ...
  Future<void> _saveRideState() async {
    try {
      if (currentRideState != RideState.initial && tripDetails != null) {
        final SharedPreferences prefs = Get.find<SharedPreferences>();
        
        // Save ride state
        await prefs.setString(ACTIVE_RIDE_STATE_KEY, currentRideState.toString());
        
        // Save trip details as JSON
        if (tripDetails != null) {
          // Store minimal trip data (id, status, key fields)
          Map<String, dynamic> rideData = {
            'trip_id': tripDetails!.id,
            'ref_id': tripDetails!.refId,
            'current_status': tripDetails!.currentStatus,
            'pickup_address': tripDetails!.pickupAddress,
            'destination_address': tripDetails!.destinationAddress,
            'driver_name': tripDetails!.driver?.firstName,
          };
          await prefs.setString(ACTIVE_RIDE_KEY, jsonEncode(rideData));
          await prefs.setString(ACTIVE_RIDE_ID_KEY, tripDetails!.id ?? '');
        }
      }
    } catch (e) {
      // Silent fail - don't interrupt ride flow
    }
  }
  ...
  Future<void> restoreActiveRide() async {
    try {
      ...
      String? rideStateStr = prefs.getString(ACTIVE_RIDE_STATE_KEY);
      String? rideId = prefs.getString(ACTIVE_RIDE_ID_KEY);
      
      if (rideStateStr != null && rideId != null && rideId.isNotEmpty) {
        // Parse saved ride state
        RideState? savedState = RideState.values.firstWhereOrNull(
          (state) => state.toString() == rideStateStr,
        );
        
        if (savedState != null && savedState != RideState.initial) {
          currentRideState = savedState;
          update();
          
          // Optionally fetch fresh trip details from API
          await _fetchTripDetails(rideId);
        }
      }
    } catch (e) {
      // Silent fail
  ...
  Future<void> _fetchTripDetails(String tripId) async {
    try {
      // Fetch current trip details from API to ensure we have latest data
      Response response = await rideServiceInterface.getRideDetails(tripId);
      if (response.statusCode == 200 && response.body['data'] != null) {
        tripDetails = TripDetailsModel.fromJson(response.body).data!;
        update();
      ...
      // Silent fail - use cached state
    }
  }

  Future<void> clearPersistedRideState() async {
    try {
      final SharedPreferences prefs = Get.find<SharedPreferences>();
      await prefs.remove(ACTIVE_RIDE_KEY);
      await prefs.remove(ACTIVE_RIDE_STATE_KEY);
      await prefs.remove(ACTIVE_RIDE_ID_KEY);
    } catch (e) {
      // Silent fail
    }
  }
