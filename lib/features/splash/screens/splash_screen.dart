...
class _SplashScreenState extends State<SplashScreen>
      ...
      Get.find<ProfileController>().getProfileInfo().then((value) {
        if (value.statusCode == 200) {
          ...
          if (Get
              ...
              .profileModel
              ?.data
              ?.phone == '+${fromCountryCode!.trim()}$fromMartPhone') {
            _route();
          } else {
  ...
  void forLoginUserRoute() {
    if (widget.notificationData != null) {
      NotificationHelper.notificationRouteCheck(
          widget.notificationData!, formSplash: true,
      ...
      Get.find<ProfileController>().getProfileInfo().then((value) {
        if (value.statusCode == 200) {
          ...
          if (value.body['data']['is_profile_verified'] == 1) {
            ...
            Get.find<RideController>().restoreActiveRide().then((_) {
              // Check if there's an active ride to resume
              if (Get.find<RideController>().currentRideState != RideState.initial) {
                // User has an active ride, navigate to ride tracking screen
                Get.offAll(() => const DashboardScreen());
                // TODO: Navigate to appropriate ride UI (map screen or ride tracking)
                // based on currentRideState
              } else {
                // No active ride, proceed to dashboard
                Get.offAll(() => const DashboardScreen());
              }
            });
          } else {
